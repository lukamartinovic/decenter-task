...
export const useRateContract = (collateralType: string) => {
    ...
    useEffect(() => {
        (async () => {
            ...
            const cachedRates = JSON.parse(sessionStorage.getItem('rates') || '[]');
            const cachedRate = cachedRates.find((rate: Rate) => rate.collateralType === collateralType);
            ...
            try {
                ...
                sessionStorage.setItem('rates', JSON.stringify([...cachedRates, rateToSave]));
                setRate(rateToSave);
            } catch (e) {
                console.error(e)
            }
